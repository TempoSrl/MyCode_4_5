	...
	public class GetData :IGetData {
		...
		public void ClearTables() {
			...
			foreach (DataTable T in DS.Tables) {
				...
				if (model.IsCached(T))
					continue;
				if (!VisitedFully.Contains(T.TableName))
		...
		void xCopyChilds(DataSet Dest, DataSet Rif, DataRow RSource) {
			...
			if (!Dest.Tables.Contains(source_unaliased))
				source_unaliased = RSource.Table.TableName;
		...
		public static bool CheckChildRel(DataRelation R) {
			...
			for (int N = 0; N < R.ParentColumns.Length; N++) {
				...
				if (QueryCreator.IsPrimaryKey(R.ParentTable, ParCol.ColumnName) &&
					 QueryCreator.IsPrimaryKey(R.ChildTable, ChildCol.ColumnName))
					linkparentkey = true;
			}
		...
		public async Task<DataRow> GetFromViewByKey(DataTable Dest, DataRow Key) {
			...
			for (int i = 0; i < TargetTable.PrimaryKey.Length; i++) {
				...
				if (!found)
					return null; //key columns were not found
			}
		...
		/// <param name="T"></param>
		/// <returns></returns>
		public static MetaExpression MergeFilters(object Filter1, DataTable T) {
			if (Filter1 is string) {
				Filter1 = q.fromString(Filter1 as string);
			}
			if (T == null) return Filter1 as MetaExpression;
		...
		public void recursivelyMarkSubEntityAsVisited(DataTable mainTable,
			...
			foreach (DataRelation Rel in mainTable.ChildRelations) {
				...
				if ((!MetaModel.IsSubEntityRelation(Rel) && model.CanClear(Rel.ChildTable))
					|| visited.Contains(childtable))
					continue; //if continue--> it will be cleared
		...
		public async Task<bool> Get(bool onlyperipherals = false, DataRow OneRow = null) {
			...
			foreach (DataTable T in DS.Tables) {
				if (visited.Contains(T.TableName))
					continue;
				if (MetaModel.IsTemporaryTable(T))
					continue;
				if (T.ExtendedProperties["RealTable"] is DataTable Main) {
					if (visited.Contains(Main.TableName))
		...
		async public Task<SelectBuilder> GetTable(DataTable table,
			...
			sortBy = sortBy ?? table.getSorting();
			...
			if (mergedfilter is null || mergedfilter.isTrue())
				VisitedFully.Add(table.TableName);
			model.TableHasBeenRead(table);
			if (selList == null)
				model.GetTemporaryValues(table, Conn.Security);
			return mySel;
		}
				...
				foreach (var tableName in toVisit) {
					...
					foreach (DataRelation Rel in T.ChildRelations) {
						...
						if (visited.Contains(childtable))
							continue;
						if (toVisit.Contains(childtable))
							continue;
						visited.Add(childtable);
					...
					foreach (DataRelation Rel in T.ParentRelations) {
						...
						if (visited.Contains(parenttable))
							continue;
						if (toVisit.Contains(parenttable))
							continue;
			...
			foreach (DataRow RR in ViewTable.Rows) {
				...
				foreach (DataColumn CC in TargetTable.Columns) {
					...
					if (cachedChildSourceColumn.ContainsKey(k)) {
						...
						if (skippedColumns.Contains(colname))
							continue;
						bool colFound = false;
						foreach (DataColumn CV in ViewTable.Columns) {
							if (CV.ExtendedProperties["ViewSource"] == null) {
		...
		DataRow getRowFromCache(DataTable t, DataColumn[] col, MetaExpression filter, out bool found) {
			...
			if (!tableCache.ContainsKey(tabKey))
				return null;
			if (!tableCache[tabKey].ContainsKey(filter.toString()))
				return null;
			found = true;
		...
		bool checkPreScannedTable(DataTable t, DataColumn[] cols) {
			...
			if (preScannedTablesRows[tabKey] > 0)
				return false;
			foreach (DataRow r in t.Rows) {
			...
			foreach (DataColumn CCT in TargetTable.Columns) {
				...
				if (cachedParentSourceColumn.ContainsKey(k)) {
					...
					foreach (DataColumn CCV in ViewTable.Columns) {
						if (CCV.ExtendedProperties["ViewSource"] == null)
							continue;
		...
		public async Task GetAllChildRows(DataRow[] RR, HashSet<string> allowed, List<SelectBuilder> SelList = null) {
			...
			foreach (DataRelation rel in currTable.ChildRelations) {
				var allowedParents = currTable.filter(QueryCreator.GetRelationActivationFilter(rel));

				var childtable = rel.ChildTable.TableName;
				if (currTable.Rows.Count == 0)
					continue;
				if (!allowed.Contains(childtable))
					continue;
				...
				if ((viewTable != null) &&
					...
					foreach (var r in RR) {
						...
						if (DataRowInList(r, allowedParents))
							await GetViewChildTable(r, rel);//allowedParents di solito ha una riga sola
					}
					continue;
				}
		...
		async Task GetChildRows(DataRow R, HashSet<string> allowed, List<SelectBuilder> selList = null) {
			...
			foreach (DataRelation Rel in R.Table.ChildRelations) {
				DataRow[] AllowedParents = R.Table.filter(QueryCreator.GetRelationActivationFilter(Rel));
				if (!DataRowInList(R, AllowedParents))
					continue;
		...
		public async Task<bool> GetParentRows(DataRow r, HashSet<string> allowed, List<SelectBuilder> selList) {
			...
			foreach (DataRelation rel in r.Table.ParentRelations) {
				...
				if (!allowed.Contains(parenttable))
					continue;
				if (QueryCreator.ContainsNulls(r, rel.ChildColumns, DataRowVersion.Default))
					continue;
