...
namespace mdl {
    ...
    public class DataAccess :MarshalByRefObject, IDisposable, IDataAccess {
	    ...
	    public virtual void startPosting(IDataAccess mainConn) {
            if (mainConn == this)
                return;
            if (mainConn.Descriptor != this.descriptor)
                throw new Exception("Can't create a transaction on two different databases");
        ...
        protected void sureClosing() {
            if (Driver == null)
                return;
            try {
                if (Driver.State == ConnectionState.Open)
                    Driver.Close();
                //nesting=0;
            }
            catch (Exception E) {
        ...
        public virtual bool Persisting {
            ...
            set {
                ...
                if (_myPersisting) {
                    ...
                    if (nesting == 0) {
                        sureClosing(); // .GetAwaiter().GetResult();
                    }
                }
                else {
        ...
        public virtual string LastError {
            get {
                if (mainConnection != null)
                    return mainConnection.LastError;
		...
		public virtual string SecureGetLastError() {
            if (mainConnection != null)
                return mainConnection.SecureGetLastError();
            return myLastError;
        }
        ...
        bool assureOpen() {
            ...
            if (DateTime.Now < nextCheck)
                return true;
            if (checkStillOpen().GetAwaiter().GetResult()) {
        ...
        public virtual async Task ReadStructures(params string[] tableNames) {
            ...
            int handle = StartTimer("ReadStructures()");
        ...
        public virtual string GetJoinSql(DataTable table1, string table2, q filterTable1, q filterTable2, string[] joinColumnsTable1, string[] joinColumnsTable2 = null) {
            ...
            var clauseJoin = q.and(from cc in joinColumnsTable1.Zip(joinColumnsTable2, (First, Second) => (First, Second))
        ...
        public virtual async Task<DataTable> CreateTable(string tablename, string columns = "*", bool addExtProp = false) {
            ...
            if (tablename == "customobject" || tablename == "columntypes" || tablename == "customtablestructure") {
                ...
                if (addExtProp)
                    await AddExtendedProperties(TT);
                return TT;
            }

            //int handle = metaprofiler.StartTimer("Inside CreateTableByName("+tablename+")");
            int handle = StartTimer("Inside CreateTableByName()");
        ...
        public virtual async Task GetViewStructureExtProperties(DataTable T) {
            if (T.tableForPosting() != T.TableName)
                return;
            int handle = StartTimer("GetViewStructureExtProperties(" + T.TableName + ")");
        ...
        public virtual async Task AddExtendedProperties(DataTable T) {
            int handle = StartTimer($"AddExtendedProperty*{T.TableName}");
        ...
        public virtual async Task<(string listType, dbstructure dbs)> GetListType(string tablename, string listtype) {
            ...
            int handle = StartTimer("GetListType*" + tablename);
            var DBS = await descriptor.GetStructure(tablename, this);
            if (listtype == null)
                return (listtype, DBS);
            ...
            if (DBS.customview.Select(CFilter).Length == 0) {
                if (descriptor.IsToRead(DBS, "customview"))
                    await SelectIntoTable(DBS.customview, filter: DBfilter);
                if (descriptor.IsToRead(DBS, "customredirect"))
                    await SelectIntoTable(DBS.customredirect, filter: DBfilter);
            }
            ...
            if (descriptor.IsToRead(DBS, "customviewcolumn"))
                await SelectIntoTable(DBS.customviewcolumn, filter: DBfilter);
            if (descriptor.IsToRead(DBS, "customvieworderby"))
                await SelectIntoTable(DBS.customvieworderby, filter: DBfilter);
            if (descriptor.IsToRead(DBS, "customviewwhere"))
                await SelectIntoTable(DBS.customviewwhere, filter: DBfilter);
        ...
        public virtual async Task<DataRow> GetFormInfo(string objectname, string edittype) {
            ...
            if (found.Length > 0)
                return found[0];
            if (descriptor.IsToRead(DBS, "customedit"))
                await SelectIntoTable(DBS.customedit, filter: filter);
        ...
        public virtual async Task<Dictionary<string, object>> ReadObject(string table, object filter = null, string expr = "*") {
            ...
            try {
                ...
                NN = StartTimer("ReadObject*" + table);
        ...
        public virtual async Task<object> ReadValue(string table, object filter = null, string expr = "*", string orderBy = null) {
            ...
            int NN = StartTimer("ReadValue*" + table);
            try {
        ...
        public virtual async Task<object> ExecuteScalarLastResult(string sql, int timeout = -1) {
            ...
            int NN = StartTimer("ExecuteScalarLastResult()");
        ...
        public virtual async Task<Dictionary<string, object>[]> ReadDictionaries(string sql, int timeout = -1) {
            ...
            int NN = StartTimer("ReadDictionaries");
        ...
        virtual public async Task<DataSet> CallSP(string sp_name, object[] parameters, int timeout = -1) {
            ...
            int NN = StartTimer("callSP " + sp_name);
            try {
        ...
        virtual public async Task CallVoidSPParams(string sp_name, DbParameter[] parameters, int timeout = -1) {
            await assureValidOpen();
            int NN = StartTimer("callVoidSP*" + sp_name);
        ...
        public static DataTable singleTableClone(DataTable T, bool copyProperties) {
            //#if DEBUG
            int handle = StartTimer("SingleTableClone");
            //#endif
            DataTable T2 = new DataTable(T.TableName) {
                Namespace = T.Namespace
            };
            ...
            //DataTable properties are always copyed
            foreach (object kt in T.ExtendedProperties.Keys)
                T2.ExtendedProperties[kt] = T.ExtendedProperties[kt];
        ...
        public virtual async Task SelectIntoTable(DataTable T,
            ...
            var handle = StartTimer($"public SelectIntoTable({T.TableName})");
        ...
        public async Task<DataTable> ExecuteQuery(string sql, int timeout = -1) {
            ...
            int handle = StartTimer("executeQuery()");
            try {
        ...
        private async Task selectIntoEmptyTable(DataTable emptyTable,
            ...
            int handle = StartTimer("selectIntoEmptyTable*" + tablename);
        ...
        public virtual async Task ExecuteQueryIntoTable(DataTable T, string sql, int timeout = -1) {
            ...
            int handle = StartTimer($"public SqlIntoTable*{T.TableName}");
            var EmptyTable = singleTableClone(T, false);
            EmptyTable.TableName = T.tableForReading();
        ...
        /// <param name="timeout">timeout in second, 0 means no timeout, -1 means default timeout</param>
        /// <returns></returns>
        public virtual async Task ExecuteQueryIntoEmptyTable(DataTable EmptyTable, string sql, int timeout = -1) {
            await assureValidOpen();

            sql = Compile(sql);

            int handle = StartTimer("SqlIntoEmptyTable*(" + sql + ")");
        ...
        public virtual async Task<Dictionary<T, S>> ReadDictionary<T, S>(string tablename,
            ...
            int handle = StartTimer("ReadDictionary*" + tablename);
        ...
        public virtual async Task<Dictionary<T, RowObject>> SelectRowObjectDictionary<T>(string tablename,
            ...
            int handle = StartTimer("SelectRowObjectDictionary*" + tablename);
            ...
            int handleFill = StartTimer("Cmd.SelectRowObjectDictionary*" + tablename);
            DbDataReader reader = null;
            try {
                reader = await Driver.ExecuteReaderAsync(cmd);
                if (reader.HasRows) {
        ...
        public virtual async Task<Dictionary<string, List<RowObject>>> RowObjectsBySql(string sql, params string[] tables) {
            ...
            int handle = StartTimer("SelectRowObjects()");
            sql = Compile(sql);
        ...
        public virtual async Task<List<RowObject>> SelectRowObjects(string tablename,
            ...
            int handle = StartTimer("RowObject_Select*" + tablename);
        ...
        public virtual async Task MultipleSelect(List<SelectBuilder> SelList) {
            ...
            int handle = StartTimer("MultipleSelect " + multitab);
            ...
            try {
                ...
                foreach (var sel in SelList) {
                    if (sel.DestTable == null) {
                        ...
                        var handleMerge = StartTimer("MultipleSelect DA.Merge*" + multitab);
        ...
        public virtual async Task<DataTable> ReadTableNoKey(string tablename, object filter = null, string columnlist = "*", string order_by = null, string TOP = null) {
            ...
            int handle = StartTimer("readFromTable*" + tablename);
            ...
            try {
                ...
                do {
                    ...
                    while (await reader.ReadAsync()) {
                        ...
                        if (packetSize == 0 || localRows.Count != packetSize)
                            continue;
    ...
    public class MultiCompare {
        ...
        public bool SameFieldsAs(MultiCompare C) {
            if (C.fields.Length != this.fields.Length)
                return false;
            for (int i = 0; i < C.fields.Length; i++) {
                if (fields[i] != C.fields[i])
                    return false;
            }
            return true;
        }
            ...
            if (multival_pos == -1) {
                ...
                for (int i = 0; i < fields.Length; i++) {
                    if (!HaveValue(C.values[i][0], i)) {
                        if (pos_diff != -1)
                            return false; //più di una differenza trovata
                        pos_diff = i;
                        continue;
                    }
                }
    ...
    public class SelectBuilder :ISelectBuilder {
        ...
        public MetaExpression filter {
            get {
                if (!(myfilter is null))
                    return myfilter;
                if (OMC != null)
                    return GetData.MergeFilters(OMC.GetFilter(), DestTable);
                return null;
            }
        ...
        public virtual bool CanAppendTo(SelectBuilder S) {
            if (this.tablename != S.tablename)
                return false;
            if (this.tablemap != S.tablemap)
                return false;
            if (this.group_by != null || S.group_by != null)
                return false;
            return true;
        }
        ...
        public virtual void AppendTo(SelectBuilder S, QueryHelper QH) {
            ...
            if (this.filter.toString() == S.filter.toString())
                return;
