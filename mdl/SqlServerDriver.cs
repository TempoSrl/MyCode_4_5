	...
	public class SqlServerDriver :IDbDriver {
		...
		public void Open() {
            ...
            if (!arithAborthSet) {
                arithAborthSet = true;
                try {
                    this.ExecuteNonQuery("SET ARITHABORT ON").GetAwaiter().GetResult();
                }
                catch {
                }
        ...
        public void ChangeDatabase(string dbName) {
            this.conn.ChangeDatabase(dbName);
		}
        ...
        public async Task DisposeCommand(IDbCommand command) {
            ...
            //if (command is IAsyncDisposable ) {
            //    await (command) disposable.DisposeAsync();
            //}
            //else {
            //    command.Dispose();
            //}
        ...
        public async virtual Task<IDbTransaction> BeginTransaction(IsolationLevel level) {
            transaction =  conn.BeginTransaction(level) as SqlTransaction;
            return transaction;
        }
