...
namespace mdl {
    ...
    public interface IMetaModel {
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        q GetNotEntityChildFilter(DataTable t);
        ...
        void SetNotEntityChildFilter(DataTable t, q filter);
    ...
    public class MetaModel :IMetaModel {
        ...
        static public bool IsSubEntityRelation(DataRelation R) {
            if (R.ExtendedProperties["isSubentity"] != null)
                return (bool)R.ExtendedProperties["isSubentity"];
            ...
            foreach (var cc in R.ParentColumns.Zip(R.ChildColumns,(First,Second)=>(First,Second))) {
        ...
        public bool CanClear(DataTable T) {
            if (T.ExtendedProperties["DenyClear"] == null)
                return true;
            return false;
        }
        ...
        public bool HasChanges(DataTable mainTable, DataRow sourceRow, bool isSubentity) {
            ...
            try {
                ...
                if (xVerifyChangeChilds(sourceRow.Table, myRow))
                    return true;
        ...
        public static bool xVerifyChangeChilds(DataTable tDest, DataRow rSource) {
            ...
            if (xVerifyRowChange(tDest, rSource))
                return true;
            ...
            foreach (DataRelation Rel in tSource.ChildRelations) {
                ...
                foreach (DataRow Child in rSource.getChildRows(Rel)) {
                    if (xVerifyChangeChilds(dest.Tables[Child.Table.TableName], Child))
                        return true;
                }
            }
            return false;
        ...
        static bool xVerifyRowChange(DataTable TDest, DataRow RSource) {
            if (RSource.RowState == DataRowState.Deleted)
                return false;
            ...
            foreach (DataColumn C in TSource.Columns) {
                if (C.IsTemporary())
                    continue;
                if (!TDest.Columns.Contains(C.ColumnName))
                    continue;
                if (TDest.Columns[C.ColumnName].IsTemporary())
                    continue;
                if (found[0][C.ColumnName].Equals(RSource[C.ColumnName]))
                    continue;
            ...
            foreach (DataRelation Rel in primaryTable.ChildRelations) {
                if (Rel.ChildTable == SourceTable) {
                    ...
                    foreach (DataColumn C in Rfound.ChildColumns) {
                        if (QueryCreator.IsPrimaryKey(SourceTable, C.ColumnName))
                            continue;
        ...
        void clearNotEntityChild(DataTable childTable) {
            SetNotEntityChildFilter(childTable, null);
        }
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public q GetNotEntityChildFilter(DataTable t) {
            return t.ExtendedProperties["NotEntityChild"] as q;
        }
        ...
        /// <param name="t"></param>
        /// <param name="filter"></param>
        public void SetNotEntityChildFilter(DataTable t, q filter) {
        ...
        public virtual void addNotEntityChildFilter(DataTable child, string relName) {
            ...
            if (IsNotEntityChild(child))
                return; //don't set if it has already been set

            if (relName == null)
                throw new ArgumentNullException(nameof(relName));

            if (!child.DataSet.Relations.Contains(relName))
                return;
            var rel = child.DataSet.Relations[relName];
            q filter = null;
            foreach (var c in rel.ChildColumns) {
                if (QueryCreator.IsPrimaryKey(child, c.ColumnName))
                    continue;
                filter = MetaExpression.and(filter, q.IsNull(c.ColumnName));
            }
        ...
        public void CalculateTable(DataTable T) {
            ...
            if (T.ExtendedProperties["CalculatedListing"] == null)
                return;
            string ListType = T.ExtendedProperties["CalculatedListing"].ToString();
            if (T.ExtendedProperties["CalculatedFunction"] == null)
                return;
        ...
        public void CalcTemporaryValues(DataRow R, ISecurity conn) {
            ...
            R.EndEdit();
            if ((toMark) && (R.RowState != DataRowState.Unchanged))
                R.AcceptChanges();
        ...
        public static bool IsTemporaryTable(DataTable T) {
            if (T.ExtendedProperties[IsTempTable] == null)
                return false;
            return true;
        }
